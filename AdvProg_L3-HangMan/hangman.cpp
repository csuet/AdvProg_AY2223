...
vector<string> readWordListFromFile(const string& filePath)
{
    ...
    string word;
    ifstream wordFile(filePath.c_str());
    if (!wordFile.is_open()) {
...
string generateHiddenCharacters(string answerWord) {
    ...
    string secretWord;
    for (int i = 0; i < (int)answerWord.length(); i++) secretWord += '-';
    return secretWord;
}
...
void updateSecretWord(string& secretWord, const char ch, const string& word)
{
    ...
    for (int i = 0; i < (int)word.length(); i++) if (word[i] == ch) secretWord[i] = ch;
}
    ...
    int& incorrectGuess, string& incorrectChars)
{
    ...
    ***/
    int ok = 0;
    for (int i = 0; i < word.length(); i++) if (word[i] == ch) ok = 1;
    if (ok)
    {
