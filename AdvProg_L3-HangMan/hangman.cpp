...
int generateRandomNumber(const int min, const int max)
{
    ...
    srand(time(0));
    int randoms = rand() % (max - min + 1) + min;
    return randoms;
}
...
bool isCharInWord(const char ch, const string& word)
{
    ...
    return (word.find(ch) >= 0 && word.find(ch) < word.length());
}
...
string chooseWordFromList(const vector<string>& wordList, int index) 
{
    ...
    string answer = "";
    const int count = sizeof(wordList) / sizeof(string);
    int random = generateRandomNumber(0, count);
