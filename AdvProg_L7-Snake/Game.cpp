...
void Game::snakeMoveTo(Position pos) {
	//  START CODE HERE
    if (pos.isInsideBox(0,0,width,height) == false || getCellType(pos) == CELL_OFF_BOARD || getCellType(pos) == CELL_SNAKE) status = GAME_OVER;
...
bool Game::canChange(Direction current, Direction next) const {
	...
	else if ((current == LEFT || current == RIGHT) && (next == LEFT || next == RIGHT)) return 0;
	else return 1;
}
...
void Game::setCellType(Position pos, CellType cellType)
{
	...
	// START CODE HERE
	//
	if(pos.isInsideBox(0,0,width,height) == true) {
